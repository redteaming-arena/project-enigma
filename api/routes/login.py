...
from api.deps import AuthUser, Database
from api.core import security
from api.core.security import verify_expired
from api.core.config import settings
from api.models import Token, Message, UserRegister, User
...
async def verify_token(user: AuthUser) -> Message:
    """test the clients token

    Args:
        current_user (AuthUser): dependency on the current client auth token
    ...
    return Message(
        ...
        data=dict(username=user.username, image=f"/avatar/{user.id}"),
    )
...
def oauth_provider_sign_in(
    provider: Literal["google"], popup: bool = False
) -> RedirectResponse:
    if provider == "google":
...
@router.get("/callback/oauth/{provider}", tags=["Auth", "Callback"])
async def callback(
    request: Request,
    provider: str,
    db: Database,
    ...
    if provider == "google":
        ...
        try:
            ...
            _ = token_data.get(
                "refresh_token"
            )  # Use .get() in case refresh_token is not present
            ...
            # NOTE: don't store the user picture in the case they change it
            #       from the one saved
            picture_url = userinfo.get("picture")

            # user does not exist create user
            if (
                user := await crud.get_user_by_email(db=db, email=userinfo["email"])
            ) is None:
                user = UserRegister(
                    email=userinfo["email"], image=picture_url, provider=provider
                )

                user: User = await crud.create_user(db=db, user_create=user)
    ...
    response.set_cookie(
        ...
        == "production",  # Set secure flag based on environment
    )
    return response
