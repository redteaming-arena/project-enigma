...
class FunctionValidator:
    ...
    async def validate_against_db(
        ...
        fn: str,
        kwargs: Dict[str, Any],
        func_type: Literal["validator", "sampler"] = "validator",
    ) -> Dict:
        """Validate function call against database metadata."""
        result = await session["judges"].find_one(
            {f"{func_type}.function.name": fn, "active": True}
        )

        if result is None:
            raise ValueError(
                f"Function '{fn}' does not exist within registered functions"
            )
        ...
        for param_name, value in kwargs.items():
            ...
            if param_info["type"] == "union":
                ...
                if not any(
                    isinstance(value, eval(t)) for t in valid_types if t != "None"
                ):
