...
class Delta(Enum):
    ...
    def update(self, prev: float, current: float) -> Delta:
        if prev < current:
            return 1
        elif prev > current:
            return -1
...
class BaseModelWithUtils(BaseModel):
    ...
    def to_dict(self, exclude_none: bool = True) -> dict:
        ...
        return self.model_dump(exclude_none=exclude_none, by_alias=False)
...
class GameMetadata(BaseModelWithUtils):
    ...
    game_rules: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Game-specific rules and settings"
    )
    custom_fields: Dict[str, str] = Field(
        default_factory=dict, description="Additional custom game settings"
    )
...
class Game(BaseModelWithUtils):
    ...
    image: Optional[HttpUrl | str] = (
        None  # Removed Union with str for clearer validation
    )
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None
...
class ModelMetadata(BaseModelWithUtils):
    ...
    model_config = ConfigDict(
        arbitrary_types_allowed=True,
        json_encoders={ObjectId: str},
        json_schema_extra={"endpoint": None, "tools": [], "config": {}},
    )
...
class GameSession(BaseModelWithUtils):
    ...
    model_config = ConfigDict(
        ...
        json_encoders={
            ObjectId: str,
            Optional[ObjectId]: lambda x: str(x) if x else None,
        },
        json_schema_extra={"id": "507f1f77bcf86cd799439011"},
    )
...
class GameSessionCreateResponse(BaseModelWithUtils):
    ...
    model_config = ConfigDict(
        arbitrary_types_allowed=True,
        json_schema_extra={"example": {"session_id": "64b2c8f9b3e3b975c9d3e8d9"}},
    )
    ...
    def from_game(cls, game: GameSession) -> "GameSessionCreateResponse":
        ...
        print(game)
        return cls(session_id=str(game.id), create_time=game.create_time)
...
class GameSessionPublic(BaseModelWithUtils):
    ...
    id: Optional[ObjectId | str] = Field(..., alias="_id")
...
class GameSessionHistoryResponse(BaseModelWithUtils):
    ...
    def calculate_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_duration": (
                total_duration / len(self.history) if self.history else 0
            ),
        }
...
class GameReadOnly(GameSessionHistoryItem):
    ...
    @classmethod
    def from_game_session(
        cls, session: GameSession, username: Optional[str] = None
    ) -> "GameReadOnly":
        ...
        return cls(
            ...
            duration=(
                (session.completed_time - session.create_time).total_seconds()
...
# TODO: Remove if possible; this is not too necessary
...
class Request(BaseModelWithUtils):
    ...
    model_config = ConfigDict(
        arbitrary_types_allowed=True, json_encoders={ClientMessage: Dict[str, Any]}
    )
...
class Leaderboard(BaseModelWithUtils):
    ...
    players: List[Score] = []
    models: List[Score] = []
    targets: Optional[List[Score]] = None
    step: int = 0

    @property
    def prev_snapshot(self) -> datetime:
        default_delta = datetime.now(UTC) - timedelta(seconds=60)
        self.last_snapshot if self.last_snapshot is None else default_delta
...
class Score(BaseModelWithUtils):
    id: Union[ObjectId, str] = Field(..., alias="_id")
    elo: float = 0.0
    delta: Delta = 0
