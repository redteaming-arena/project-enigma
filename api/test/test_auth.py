...
async def test_signup_username():
    ...
    user_data = {"username": username, "password": "68656c6c6f776f726c6"}

    # First signup attempt
    response = client.post(
        "/signup", json=user_data  # Send as JSON to match FastAPI's automatic parsing
    )
    ...
    response = client.post("/signup", json=duplicate_data)
    assert (
        response.status_code == status.HTTP_400_BAD_REQUEST
    ), "Expected duplicate username to fail"
    assert "already exists" in response.json().get(
        "detail", ""
...
async def test_login_username():
    ...
    # Verify the token
    response = client.get(
        "/verify-token", headers={"Authorization": f"Bearer {token['access_token']}"}
    )
    assert response.status_code == 200
    assert response.json().get("data", False)
