...
class Client:
    def __init__(
        ...
        **kwargs
        ...
        if OPENAI_MODULE_AVAILABLE and provider in ("openai", "google"):
            ...
            self._strategy = CompletionFactory.create_strategy("openai", self._client)
        if ANTHROPIC_MODULE_AVAILABLE and provider == "anthropic":
            self._client = Anthropic(api_key=api_key)
            self._strategy = CompletionFactory.create_strategy(
                "anthropic", self._client
            )
    ...
    def generate(
        ...
        return self._strategy.create_completion(
            messages=messages, stream=stream, model=model, **kwargs
        )
