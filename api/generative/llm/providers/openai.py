...
class OpenAICompletionResponse(
    ...
    def iter_chunks(self) -> Iterable[str]:
        if hasattr(self._response, "choices") and self._response.choices:
            if (
                hasattr(self._response.choices[0], "message")
                and self._response.choices[0].message
            ):
...
class OpenAICompletionStrategy(CompletionStrategy):
    ...
    def create_completion(
        ...
        return (
            OpenAICompletionStream(response, metadata)
            if stream
            else OpenAICompletionResponse(response, metadata)
        )
